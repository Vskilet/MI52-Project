...
void USART2_IRQHandler(void)
{
	...
	if ((status & USART_SR_RXNE) && (cr1 & USART_CR1_RXNEIE) ){
		if (uart2.RxCount < uart2.RxSize){
			*uart2.pRxBuffer++ = uart2.instance->DR;
			uart2.RxCount++;
		}
		else {
			/* stop RXNE interrupt not needed anymore */
			uart2.instance->CR1 &= ~USART_CR1_RXNEIE;
		}
	}
}
